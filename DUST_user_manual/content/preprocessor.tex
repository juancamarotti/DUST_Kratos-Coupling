...
the user afterwards to specify post process analyses.
...
% === ee array ===
ee = zeros(n_elems,4) ; ie = 0 ; % Initialization to zero
...
A representation of sections and regions employed for the generation of a wing is presented in Fig.~\ref{fig:parametric_wing}. 
...
file \ref{file:parametric_example_file.in} and depicted in Fig.~\ref{fig:parametric_sections} 
but generated as lifting lines is presented in Fig.~\ref{fig:parametric_sections_ll}. 
...
CAD files and an external meshing tool will generate points in a certain reference frame. 
...
\includegraphics[width=.70\textwidth]{pointwise_3_component_flip}
    ...
    reference frame: the comparison between the geometry with 
