\chapter{DUST Post-processor}
\label{ch:Post}

The DUST post-processor is used to generate meaningful data from the binary 
...
The post-processor takes the specified results and use them to obtain a variety of 
...
in the desired folder. The input file containing all the required information 
for the execution of the post-processor must be passed as argument to the command call. 
...
The input file of the post-processor contains first all the information 
...
Base name (with path) of the data which must be analyzed.
...
files with respect to the surface solution. It affects only the \texttt{.vtk} output and 
...
\opt{tecplot} for Tecplot \texttt{.plt} files.
...
100-time step analysis with the local reference frame \param{Wing1\_Reference} 
...
\subsection{Output .dat file}
The output file containing hinge loads measurements in \texttt{.dat} 
...
for a 20-time step analysis with the hinge  reference frame \param{Wing1\_Aileron} 
aligned with the global reference frame (the rotation matrix is equal to the identity)
...
required for the analysis. As an example, for a 100-time step analysis 
these lines read
\begin{inputfile}[frame=single]
...
or \opt{tecplot} for Tecplot \texttt{.plt} files
...
first and the last of the 100 time steps of the analysis reads
\begin{inputfile}[frame=single]
...
\begin{itemize}
    ...
    is the unit vector ``in the cross-flow direction''
