...
of the geometry, run the simulation in the desired condition and then post-process 
...
performed by the post-processor, which employs the global results to generate a 
...
\caption{Description of the workflow with DUST}
\label{fig:worflow}
\end{figure}
...
\begin{itemize}
    ...
    \item Logical are introduced as "T" for true and "F" for false
...
The hdf5 format is a common open-source binary format which allows to store 
...
can give a brief insight on some results). The post-processor can interpret those results 
and output processed data readily employable by the user.

Most of the post-processed results can be obtained in Tecplot binary (\texttt{}) format, 
which is compact and convenient. However Tecplot is a proprietary software available only 
under license. For this reason all the results can also be produced in the standard vtk 
